...
int Feasible(tour_t tour, city_t city) {
   city_t last_city = Last_city(tour);
   int lengthPath;
   int i,test,minEdge,bound 
   
   lengthPath = Tour_cost(tour) + Cost(last_city,city);
   if(City_count(tour) == n-1){
   	bound = lengthPath+cost(city);
   	else{
   	included = *int malloc(100);
   	for(i=0; i<n;i++){
   		if(Visited(tour,i){
   		included[i] =TRUE;
   		}
	...
	minEdge = INFINITY;
	for(i = 1; i<n; i++){
		if(!included[i]&&Cost(city,i)<minEdge){
		minEdge = Cost(city,i);
		}
		}
		bound = LengthPath_minEdge;
		included[0]=FALSE;
		for (test = 1; test<n;test++){
		 if(!included[test]){
		 	minEdge = INFINITY;
			for (i=0; i<n;i++){
				if(!included[i] &&test!=i && Cost(city,test)<minEdge){
				minEdge = Cost(city,test);
				}
				}
				included[0] =TRUE;
				included[city]=FALSE;	 
   if (bound < Tour_cost(best_tour))
      return TRUE;
   else
      return FALSE;
}  /* Feasible */
