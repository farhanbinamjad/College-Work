/* FILE:  greetings.c -- greetings program
 * Compile by:  mpicc -o greetings greetings.c
 * Run by:      qsub qsub.greetings
 *
...
main(int argc, char* argv[]) {
    ...
    int		length;	       /* size of the hostname */
    char        message[100];  /* storage for message  */
    char	name[100];     /* storage for the host */
	...
	MPI_Get_processor_name(name, &length);
        sprintf(message, "Greetings from process %d on %s!",
            my_rank,name);
        dest = 0;
