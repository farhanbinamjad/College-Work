...
void  *producerWork(void * args) {
  ...
  while (TRUE) {
    ...
    nextRowNumber = nextRowNumber+blockSize;
    ...
    blockOfWork.rowEnd = rowNumber+blockSize-1;    
    if (blockOfWork.rowEnd >= rows){
      blockOfWork.rowEnd = rows-1;
    }


    for(i=blockOfWork.rowStart;i<=blockOfWork.rowEnd;i++){
...
void  *consumerWork(void * args) {
  ...
  while (TRUE) {
    ...
    for(i=rowNumber;i<=blockOfWork.rowEnd;i++){
      addRows(A[i], B[i], Sum[i], columns);
    }

    pthread_mutex_lock(&rowsConsumedCountLock);
    rowsConsumedCount=rowsConsumedCount+(blockOfWork.rowEnd-blockOfWork.rowStart+1);
