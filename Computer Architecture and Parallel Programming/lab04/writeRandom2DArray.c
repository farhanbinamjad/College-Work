...
void read2DArray(int * rows, int * columns, double *** array2DRead, 
	     FILE * inputFilePtr) {
 	int c;
	// Do I have to move the pointer to get the next element?
	fread(row, sizeof(int), 1,inputFilePtr);
	fread(column, sizeof(int),1,Ofile);
	for(r=0; r<row; r++){
		fread(array2DRead[r], sizeof(double), column,inputFilePtr);
	        }//end of for loop 
	return row, column, array2DRead;	
